...
const processConstants = (ast, name) => {
  ...
  const newImport = parseImport(
    "import {" + upperCase(snakeCase(name)) + "} from './constants'"
  );
...
const processSwitch = (ast, name) => {
  ...
  let newFunction = parseNode(`function hola (action){
           switch (action) {
      case ${upperCase(snakeCase(name))}:
        return ${camelCase(snakeCase(name))}(state, payload);
        
      case ${upperCase(snakeCase(name))} + _SUCCESS:
        return ${camelCase(snakeCase(name))}Success(state, payload);
  
      case ${upperCase(snakeCase(name))} + _ERROR:
        return ${camelCase(snakeCase(name))}Error(state, payload);
    }
  }`);
